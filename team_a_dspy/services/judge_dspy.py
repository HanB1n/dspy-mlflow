...
class JudgeDSPY:
    ...
    def _evaluate_query_dsl_syntax(self, generated_query_dsl: dict) -> dict:
        ...
        Args:
            generated_query_dsl (dict): The generated Elasticsearch Query DSL to be validated.
        Returns:
    ...
    def _extract_query_intent(self, nl_query: str):
        """
        Extracts the core intent from the natural language query using a language model.
        
        Args:
            nl_query (str): The natural language query from which to extract the intent.
        Returns:
            list: A list of key search terms or concepts extracted from the natural language query.
        """
    ...
    def _aggregate_es_documents(self, raw_es_documents: list[dict]) -> dict[str, Any]:
        """
        Aggregates the documents retrieved from Elasticsearch to a consistent format for evaluation.
        
        Args:
            raw_es_documents (list[dict]): The raw documents returned from an Elasticsearch query.
        Returns:
            dict[str, Any]: Normalized documents plus field-level aggregate summaries.
        """
        ...
        for raw_doc in raw_es_documents:
            ...
            #     {
            #         "record_id": normalized.get("RecordId"),
            #         "doc_id": normalized.get("V2DocId"),
            #         "title": _safe_get(normalized, "V2ExtrasXML", "Title"),
            #         "themes": themes,
            #         "persons": persons,
            #         "orgs": orgs,
            #         "locations": locations,
    ...
    def compute_relevance_score(self, nl_query: str, aggregation: dict[str, Any]) -> dict[str, Any]:
        """
        Evaluates the relevance of aggregated search results against the original natural language query.
        Uses an LLM to assess how well the results align with the query intent.
        
        Args:
            nl_query (str): The original natural language query.
            aggregation (dict): The aggregation output from _aggregate_es_documents.
        
        Returns:
            dict[str, Any]: A dictionary containing:
                - relevance_score: 0-100 score indicating result relevance
                - entity_coverage: Assessment of persons, orgs, locations found
                - theme_alignment: Assessment of theme relevance
                - tone_alignment: Assessment of tone/sentiment match
                - reasoning: Detailed explanation from the LLM
        """
        relevance_signature = dspy.ChainOfThought(RelevanceScorerSignature)
        
        with dspy.context(lm=self.lm):
            # Format aggregation summary for the LLM
            agg_summary = {
